...
def render(data: dict, week_id: str) -> None:
    ...
    with col_sent:
        ...
        st.markdown(
            '<div class="card-title">Sentiment Distribution by Theme</div>',
            unsafe_allow_html=True,
        )
    ...
    with col_impact:
        st.markdown('<div class="content-card">', unsafe_allow_html=True)
        st.markdown('<div class="card-title">Theme Impact Scores</div>', unsafe_allow_html=True)
    ...
    with col_dist:
        st.markdown('<div class="content-card">', unsafe_allow_html=True)
        st.markdown('<div class="card-title">Rating Distribution</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1.5fr;
                    gap:0.5rem;padding:0.5rem 0;border-bottom:1px solid #2a2d3e;
                    margin-top:0.5rem;">
            ...
            st.markdown(
                ...
                <div style="display:flex;align-items:center;gap:0.6rem;padding:0.6rem 0;">
                    <div style="width:30px;height:30px;border-radius:8px;
                                background:rgba({_hex_to_rgb(row['color'])},0.15);
                                display:flex;align-items:center;justify-content:center;
                                font-size:0.9rem;flex-shrink:0;">
                        {row['icon']}
                    </div>
                    <div style="font-size:0.85rem;font-weight:500;color:#e2e8f0;">{row['name']}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        with col_imp:
            ...
            st.markdown(
                ...
                <div style="padding:0.6rem 0;">
                    <div style="height:4px;background:#2a2d3e;border-radius:2px;overflow:hidden;margin-bottom:3px;">
                        <div style="width:{min(score_pct*2,100):.0f}%;height:100%;background:{bar_color};border-radius:2px;"></div>
                    </div>
                    <div style="font-size:0.75rem;color:#94a3b8;">{score_pct:.1f}%</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
